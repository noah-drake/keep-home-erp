...
function TransactionEngine() {
  ...
  // Capture URL params to auto-fill context
  ...
  const [stockByLoc, setStockByLoc] = useState<any[]>([]) // "Live Ledger" for validation
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const [matRes, locRes, sblRes] = await Promise.all([
        // Fetch Active Materials (Global + Local)
        ...
        // Fetch Live Stock for Validation
  ...
  // Initialize Lines based on URL Params (The "Context" Fix)
  useEffect(() => {
    if (!loading && lines.length === 0) {
      // Logic: If coming from a Location Master with no stock, assume INBOUND.
      // If coming from Item Master, leave blank for user choice.
      let defaultType = ''
      if (urlLocationId) {
         // Check if this location has ANY stock of ANY item
  ...
  const validateBatch = () => {
    ...
    for (const line of lines) {
      if (!line.type || !line.material_id || !line.location_id || !line.quantity) {
        alert("VALIDATION FAILED: All fields (Operation, Item, Store, Qty) are required.")
        return false
      }
      ...
      // Check Stock Availability for OUTBOUND/TRANSFER
    ...
    for (const [key, totalRequested] of projectedChanges.entries()) {
      ...
      if (totalRequested > available) {
        const matName = materials.find(m => m.id === mId)?.name || "Item"
        const locName = locations.find(l => l.id === lId)?.name || "Location"
        alert(`INSUFFICIENT STOCK: You are trying to move ${totalRequested} of "${matName}" from "${locName}", but only ${available} exist.`)
        return false
      }
    }
    return true
  ...
  if (loading) return <div className="min-h-screen bg-black flex items-center justify-center text-purple-500 font-black uppercase tracking-widest">Loading Ledger...</div>
    ...
    <div className="min-h-screen bg-[#0a0a0a] p-4 md:p-8 text-white font-sans pb-32">
      <div className="max-w-[1440px] mx-auto space-y-8">
        ...
        <div className="space-y-3">
          {lines.map((line) => {
             // Logic: If OUTBOUND/TRANSFER, only show locations with that item in stock. 
             // If INBOUND, show ALL locations.
            ...
            return (
              <div key={line.id} className="bg-[#0f0f0f] border border-gray-800 p-4 rounded-[1.5rem] relative group shadow-xl hover:border-gray-700 transition-colors">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 items-end">
                  
                  {/* OPERATION */}
                  <div className="lg:col-span-2">
                    <label className={lbl}>Operation</label>
                    <select value={line.type} onChange={e => updateLine(line.id, 'type', e.target.value)} className={`inpt text-[10px] ${line.type === 'INBOUND' ? 'border-purple-500 text-purple-400' : line.type === 'OUTBOUND' ? 'border-yellow-500 text-yellow-500' : line.type === 'TRANSFER' ? 'border-blue-500 text-blue-400' : 'border-gray-800 text-gray-500'}`}>
                  ...
                  <div className={line.type === 'TRANSFER' ? 'lg:col-span-2' : 'lg:col-span-3'}>
                    ...
                    <select required value={line.location_id} onChange={e => updateLine(line.id, 'location_id', e.target.value)} className="inpt text-xs">
                      <option value="" className="bg-gray-900 text-white">-- Select --</option>
                      {validLocations.map(l => <option key={l.id} value={l.id} className="bg-gray-900 text-white">{l.name}</option>)}
                    </select>
                  </div>
                ...
                {line.showNotes && <div className="mt-4 animate-in fade-in slide-in-from-top-2"><input placeholder="Transaction note..." value={line.notes} onChange={e => updateLine(line.id, 'notes', e.target.value)} className="inpt text-xs italic" /></div>}
